...
class SEMMethodSession:
    ...
    def _initial_snapshot(treatment_id: str) -> MemoryGraphSnapshot:
        ...
        if treatment_id == "flat_episodic":
            nodes = (
                MemoryNodeRecord(
                    ...
                    purpose="append grounded episodic evidence",
                    scope="assignment",
                    mode="APPEND",
                    schema={"type": "object", "required": ["text", "source"]},
                    access=("MEMORY_ASK", "NODE_DISCOVERY"),
                    sources=("environment.evidence", "agent.step"),
                    transform={"kind": "identity", "version": "sem.v1"},
                    maintenance_contract={"retention": "append-only", "evolution": "none"},
                    provenance={"owner": "SEM", "treatment": "flat_episodic"},
                ),
            )
        ...
        nodes = (
            MemoryNodeRecord(
                ...
                purpose="grounded current world state and task context",
                scope="assignment",
                mode="CURRENT",
                schema={"type": "object", "required": ["observation", "task_id"]},
                access=("STATE_READ", "MEMORY_ASK", "NODE_DISCOVERY"),
                sources=("environment.observation", "agent.step"),
                transform={"kind": "state_projection", "version": "sem.v1"},
                maintenance_contract={"updates": "replace-current", "evidence": "append-only"},
                provenance={"owner": "SEM", "treatment": "canonical_initial_state"},
            ),
            MemoryNodeRecord(
                "memory:episode", "episode", "episode",
                "Action and interaction episodes.", "g0",
                parent_ids=("memory:context",),
                purpose="append action and interaction episodes",
                scope="assignment",
                mode="APPEND",
                schema={"type": "object", "required": ["task_id", "action"]},
                access=("MEMORY_ASK", "NODE_DISCOVERY"),
                sources=("agent.step", "environment.effect"),
                transform={"kind": "episode_append", "version": "sem.v1"},
                maintenance_contract={"retention": "append-only", "evolution": "semantic-only"},
                provenance={"owner": "SEM", "treatment": "canonical_initial_state"},
            ),
            MemoryNodeRecord(
                ...
                purpose="append verified task outcomes and effects",
                scope="assignment",
                mode="APPEND",
                schema={"type": "object", "required": ["task_id", "success", "failure_reason"]},
                access=("MEMORY_ASK", "NODE_DISCOVERY"),
                sources=("environment.effect", "method.outcome"),
                transform={"kind": "outcome_append", "version": "sem.v1"},
                maintenance_contract={"retention": "append-only", "evolution": "semantic-only"},
                provenance={"owner": "SEM", "treatment": "canonical_initial_state"},
            ),
        )
    ...
    def _build_create(self, demand: StructuralDemand) -> tuple[SemanticEdit, ...]:
        ...
        payload = {
            ...
            "purpose": f"aggregate semantic evidence for {demand.family}",
            "scope": "assignment",
            "mode": "AGGREGATE",
            "schema": {"type": "object", "required": ["family", "signal", "evidence_ids"]},
            "access": ("MEMORY_ASK", "NODE_DISCOVERY"),
            "sources": ("memory:outcome",),
            "transform": {"kind": "semantic_projection", "version": "sem.v1"},
            "maintenance_contract": {"evidence": "append-only", "evolution": "candidate-only"},
            "provenance": {"owner": "SEM", "edit_family": "CREATE"},
        }
        edits = (
            SemanticEdit(
                "edit:create:" + node_id,
        ...
        for new_id, suffix in ((left_id, "verified"), (right_id, "unresolved")):
            edits.append(SemanticEdit(
                f"edit:create:{new_id}", "create", new_id,
                {
                    ...
                    "purpose": f"{base.purpose} ({suffix} split)",
                    "scope": base.scope,
                    "mode": base.mode,
                    "schema": dict(base.schema),
                    "access": base.access,
                    "sources": base.sources,
                    "transform": {**dict(base.transform), "split_branch": suffix},
                    "maintenance_contract": dict(base.maintenance_contract),
                    "provenance": {**dict(base.provenance), "edit_family": "SPLIT"},
                },
        ...
        edits = [
            SemanticEdit(
                "edit:create:" + target_id, "create", target_id,
                {
                    ...
                    "evidence_ids": demand.evidence_ids,
                    "parent_ids": ("memory:outcome",),
                    "purpose": f"aggregate merged semantic evidence for {demand.family}",
                    "scope": "assignment",
                    "mode": "AGGREGATE",
                    "schema": {"type": "object", "required": ["family", "signal", "evidence_ids"]},
                    "access": ("MEMORY_ASK", "NODE_DISCOVERY"),
                    "sources": tuple(source_ids),
                    "transform": {"kind": "semantic_merge", "version": "sem.v1"},
                    "maintenance_contract": {"evidence": "append-only", "evolution": "candidate-only"},
                    "provenance": {"owner": "SEM", "edit_family": "MERGE"},
                },
            ...
            "nodes": [
                {
                    ...
                    "purpose": node.purpose, "scope": node.scope, "mode": node.mode,
                    "schema": dict(node.schema), "access": list(node.access),
                    "sources": list(node.sources), "transform": dict(node.transform),
                    "maintenance_contract": dict(node.maintenance_contract),
        ...
        return MemoryGraphSnapshot(
            ...
            tuple(
                MemoryNodeRecord(
                    ...
                    str(row.get("purpose", "general")),
                    str(row.get("scope", "global")),
                    str(row.get("mode", "APPEND")),
                    dict(row.get("schema", {})),
                    tuple(row.get("access", ())),
                    tuple(row.get("sources", ())),
                    dict(row.get("transform", {})),
                    dict(row.get("maintenance_contract", {})),
