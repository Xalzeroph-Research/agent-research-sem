...
| Universal method execution | noetrium.platform.bind_universal_method_machine, run_method_program, run_method_program_async | Complete: every assignment is hosted as a bounded UMM program; SEM supplies only the assignment node | Keep the platform commit explicit in run/artifact provenance |
...
| Checkpoint/branch/evaluation | experimentation/checkpoint, branch, evaluation + noetrium.platform.bind_method_checkpoint_store | UMM method checkpoints are crash-durable and resume-validated; scripted-environment checkpoints are sealed into result artifacts; live environment checkpoints remain optional until a deployment provider is bound | Bind the live branch/world provider into the run manifest |
